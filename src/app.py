...
# This dictionary keeps track of the system state, battery stats, and custom limits.
CONFIG = {
    "low_limit": 20,
    "high_limit": 80,
    "mute_alarms": False,
...
# =========================================================================
# 🛠️ SECTION 1: NATIVE TERMUX API ACTIONS
# =========================================================================

def get_battery_status():
    """Queries the native Termux API command to get phone battery hardware stats."""
    try:
...
def trigger_silent_update(level):
    """Sends a low-priority Android notification card and speaks the level shift out loud."""
...
def trigger_critical_alarm(title, message):
    """Fires a persistent system alert card, vibrates the hardware, and loops custom/fallback audio."""
    if CONFIG["mute_alarms"]:
        return
...
def clear_critical_alarm():
    """Cleans up active warning card states and completely cuts off hardware audio players."""
...
# =========================================================================
# 🔄 SECTION 2: BACKGROUND LOOP MOTOR (Runs constantly in the background)
# =========================================================================

def battery_monitor_loop():
    """Independent infinite loop running quietly in the background to enforce threshold logic."""
    ...
    while True:
        ...
        if status:
            ...
            # Condition 1: Low Battery Boundary Alarm
            ...
            # Condition 2: High Battery Boundary Alarm
            ...
            # Condition 3: Safe Mid-Range Tracking Updates (TTS tracking per 1% change)
            else:
        ...
        # Check faster (every 5s) if alarm is ringing to repeat vibration, otherwise rest for 15s
        time.sleep(5 if (CONFIG["low_alarm_active"] or CONFIG["high_alarm_active"]) else 15)


# =========================================================================
# 🎨 SECTION 3: FRONTEND USER INTERFACE (HTML & STYLING)
# =========================================================================
        ...
        function updateDashboard() {
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    // Update numeric entry inputs from server state config values
                    if (document.activeElement !== document.getElementById('low-limit')) {
                        document.getElementById('low-limit').value = data.low_limit;
                    }
                    if (document.activeElement !== document.getElementById('high-limit')) {
                        document.getElementById('high-limit').value = data.high_limit;
                    }

let muteBtn = document.getElementById('mute-btn');if (data.mute_alarms) {muteBtn.innerText = "🔇 Alerts Muted";muteBtn.className = "btn btn-mute btn-muted";} else {muteBtn.innerText = "🔊 Alerts Enabled";muteBtn.className = "btn btn-mute";}});}function saveLimits() {let low = document.getElementById('low-limit').value;let high = document.getElementById('high-limit').value;fetch('/api/config', {method: 'POST',headers: { 'Content-Type': 'application/json' },body: JSON.stringify({ low_limit: parseInt(low), high_limit: parseInt(high) })}).then(() => alert("Configuration settings saved successfully!"));}function toggleMute() {fetch('/api/mute', { method: 'POST' }).then(() => updateDashboard());}setInterval(updateDashboard, 3000);updateDashboard();"""=========================================================================🌐 SECTION 4: WEB APPLICATION APIS & SERVER RUNTIME=========================================================================@app.route('/')def index():"""Serves the complete frontend dashboard UI to your browser."""return render_template_string(DASHBOARD_HTML)@app.route('/api/status', methods=['GET'])def get_status():"""Endpoint providing real-time data readings in clean JSON format."""return jsonify(CONFIG)@app.route('/api/config', methods=['POST'])def set_config():"""Receives new high and low threshold settings from the webpage fields."""data = request.jsonCONFIG["low_limit"] = data.get("low_limit", CONFIG["low_limit"])CONFIG["high_limit"] = data.get("high_limit", CONFIG["high_limit"])return jsonify({"status": "success"})@app.route('/api/mute', methods=['POST'])def toggle_mute():"""Instantly switches on/off all text-to-speech engine outputs and active alarms."""CONFIG["mute_alarms"] = not CONFIG["mute_alarms"]if CONFIG["mute_alarms"]:clear_critical_alarm()return jsonify({"status": "success"})if name == "main":# 1. Start checking the battery state background threadmonitor_thread = threading.Thread(target=battery_monitor_loop, daemon=True)monitor_thread.start()# 2. Start the local server endpoint interface on Port 5000app.run(host='0.0.0.0', port=5000, debug=False)
